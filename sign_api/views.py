...
from sign_api.models import Event, Guest
...
def get_event_list(request):
    ...
    if eid != '':
        ...
        if name != '':
            datas = []
            results = Event.object.filter(name__contains=name)
            if results:
                for r in results:
                    event = {}
                    event['name'] = r.name
...
def add_guest(request):
    ...
    if not result:
        return JsonResponse({'status': 10023, 'message': 'event status is nit available!'})

    event_limit = Event.objects.get(id=eid).limit           # 发布会限制人数
    guest_limit = Guest.objects.filter(event_id=eid).limit  # 发布会已添加的嘉宾数
